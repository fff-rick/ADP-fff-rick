...
2. 基于前一轮确定的页面稿，将首页定位为“AI 运维指挥台”，采用 `Industrial Editorial Command Center` 视觉方向，重点突出：
   ...
   - 案例库
   - 审计回放
3. 在 `internal/api/server.go` 中新增路由：
   - `GET /`：返回控制台首页
   - `GET /static/`：提供静态资源
   - `GET /api/v1/dashboard/summary`：提供首页聚合数据接口
4. 新增 `internal/api/ui.go`，通过 Go `embed` 嵌入前端资源，避免单独部署静态目录。
5. 新增前端页面资源：
   - `internal/api/ui/index.html`
   - `internal/api/ui/app.css`
...
7. `app.js` 已接入现有后端接口，支持：
   ...
   - 执行最近一次诊断计划
   - 在首页直接批准/拒绝待审批任务
8. 为避免首页一次性请求过多零散接口，新增 `internal/api/dashboard.go`，将以下数据汇总成单个响应：
   - `MetricsSnapshot`
   - 最近任务
   - Worker 列表
   - 待审批任务
   ...
   - 最近审计日志
   - 模板总数
9. 新增 `internal/api/ui_test.go`，补充 UI 层验证，覆盖：
   - `GET /` 返回首页 HTML
...
```text
internal/api/dashboard.go                  -- 首页聚合数据接口
internal/api/ui.go                         -- 嵌入式 UI 资源与静态文件服务
internal/api/ui/index.html                 -- 控制台首页结构
internal/api/ui/app.css                    -- 控制台视觉样式
internal/api/ui/app.js                     -- 控制台交互逻辑
internal/api/ui_test.go                    -- 首页与聚合接口测试
```

## 本次修改文件

```text
internal/api/server.go                     -- 注册首页、静态资源与 dashboard summary 路由
```
...
- `gofmt -w internal/api/server.go internal/api/dashboard.go internal/api/ui.go internal/api/ui_test.go`
- `$env:GOCACHE='C:\Users\94282\Documents\work\ADP-fff-rick\.gocache'; go test ./internal/api`
